...
public class BrowseAndSelectController implements CleanupControl {
	private static final Logger LOG = Logger.getLogger("BrowseAndSelectController.class.getName()");
	...
	class SelectCatalogListener implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			...
			// which means no row was selected
				...
				JOptionPane.showMessageDialog(catalogListWindow, errMsg, "Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	...
	class SelectProductListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			// Value of selectedRow is -1, which means no row was selected
				...
				String errMsg = "Please select a row.";
				JOptionPane.showMessageDialog(productListWindow, errMsg, "Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	class QuantityOkListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			// in Quantity
			//
			String quantityRequested = quantityWindow.getQuantityDesired();
			Quantity quantity = new Quantity(quantityRequested);
			IProductSubsystem productSubsystem = new ProductSubsystemFacade();
			try {
				productSubsystem.readQuantityAvailable(productDetailsWindow.getName(), quantity);
	...
	// ProductListWindow
	public ActionListener getSelectProductListener(ProductListWindow w, boolean useDefData) {
	...
	private JInternalFrame[] internalFrames = { productListWindow, catalogListWindow, productDetailsWindow,
			quantityWindow, cartItemsWindow, selectOrderWindow, maintainCatalogTypes, maintainProductCatalog };
