...
class DbClassOrder implements IDbClass {
	...
	private void populateOrderItems(ResultSet rs) {
		...
		try {
			while (rs.next()) {
				IOrderItem orderItem = new OrderItem(rs.getInt("lineitemid"), rs.getInt("productid"),
						rs.getInt("orderid"), rs.getString("quantity"), rs.getString("totalPrice"));
	...
	private void populateOrderData(ResultSet resultSet) throws DatabaseException {
    	...
    	try {
			if(resultSet.next()){
				orderData = new Order(resultSet.getInt("orderId"),resultSet.getString("orderDate"),resultSet.getString("totalPrice"));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
